                             ...
                             bplus_node<K, V> *right, K key, int level)
{
    if (!left->parent && !right->parent) {
        /* new parent */
        bplus_non_leaf<K, V> *parent = non_leaf_new<K, V>();
        parent->key[0] = key;
                           ...
                           bplus_node<K, V> *l_ch, bplus_node<K, V> *r_ch, K key, int level)
{
    ...
    if (node->count == tree->order) {
        ...
        bplus_non_leaf<K, V> *sibling = non_leaf_new<K, V>();
        if (insert < split) {
...
static int leaf_insert(bplus_tree<K, V> *tree, bplus_leaf<K, V> *leaf, K key, V data)
{
    ...
    if (leaf->count == tree->entries) {
        ...
        bplus_leaf<K, V> *sibling = leaf_new<K, V>();
...
static int bplus_tree_insert(bplus_tree<K, V> *tree, K key, V data)
{
    ...
    /* new root */
    bplus_leaf<K, V> *root = leaf_new<K, V>();
    root->key[0] = key;
    root->data[0] = data;
    // hash
    hash_leaf(root);
...
template <typename K, typename V>
static int bplus_tree_delete(bplus_tree<K, V> *tree, K key)
{
    bplus_node<K, V> *node = tree->root;
    while (node) {
        if (is_leaf(node)) {
...
template <typename K, typename V>
bplus_tree<K, V> *bplus_tree_init(int order, int entries)
{
...
template <typename K, typename V>
void bplus_tree_deinit(bplus_tree<K, V> *tree)
{
    int i;
...
template <typename K, typename V>
int bplus_tree_get_range(bplus_tree<K, V> *tree, K key1, K key2)
{
    int i, data = 0;
    K min = key1 <= key2 ? key1 : key2;
    K max = min == key1 ? key2 : key1;
    bplus_node<K, V> *node = tree->root;
...
template <typename K, typename V>
static inline int children(bplus_node<K, V> *node)
{
    return ((bplus_non_leaf<K, V> *) node)->count;
}

template <typename K, typename V>
static void node_key_dump(bplus_node<K, V> *node)
{
    int i;
    if (is_leaf(node)) {
        for (i = 0; i < node->count; i++) {
            printf("%d ", ((bplus_leaf<K, V> *)node)->key[i]);
        }
    } else {
        for (i = 0; i < node->count - 1; i++) {
            printf("%d ", ((bplus_non_leaf<K, V> *)node)->key[i]);
        }
    }
    printf("\n");
}
...
template <typename K, typename V>
static K node_key(bplus_node<K, V> *node, int i)
{
    if (is_leaf(node)) {
        return ((bplus_leaf<K, V> *)node)->key[i];
    } else {
        return ((bplus_non_leaf<K, V> *)node)->key[i];
    }
}
...
template <typename K, typename V>
void bplus_tree_dump(bplus_tree<K, V> *tree)
{
    int level = 0;
    bplus_node<K, V> *node = tree->root;
    node_backlog<K, V> *p_nbl = nullptr;
    node_backlog<K, V> nbl_stack[BPLUS_MAX_LEVEL];
    node_backlog<K, V> *top = nbl_stack;
